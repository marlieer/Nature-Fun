...
@section('content')
    ...
    <div class="container">
        <form method="POST" action="/registrationAsAdmin">
            ...
            <input id="searchChild" onkeyup="myFunction()" class="form-control" type="text" placeholder="Search"
                   aria-label="Search"/>
            <ul id="children">
                    ...
                    <li style="display: none;">
                        <div>
                            <input type="checkbox" value="{{ $child->id }}"
                                   name="{{ $child->id }}"/>{{ $child->name }} {{ $child->last_name }}
                        </div>
                    </li>
                @endforeach
            </ul>
            ...
            <a href="/session" style=" width:40%; background-color:grey" class="btn btn-secondary">Cancel</a>
        </form>
    </div>

    <hr>

    <button style="background-color: #070F16; color:white;" id="registerManually" onclick="showForm()"> Not in the
        System? Register Manually
    </button>
    <br>
